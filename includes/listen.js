module.exports = function ({ api, models }) {
  setInterval(function () {
    ...
    setInterval(async () => {
      ...
      try {
        if (day != day_now) {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    for (const item of checktt.day) {
                        const userName = await Users.getNameUser(item.id) || '𝙉𝘼𝙈𝙀 𝘿𝙊𝙀𝙎 𝙉𝙊𝙏 𝙀𝙓𝙄𝙎𝙏';
                    ...
                    let checkttBody = '==𝙏𝙊𝙋 𝙄𝙉𝙏𝙀𝙍𝘼𝘾𝙏𝙄𝙊𝙉==\n\n';
            ...
            await new Promise(async resolve => {
                if (day_now == 1) {
                    console.log('Start checking new week's interactions');
                    for (const checkttFile of checkttData) {
                        const checktt = JSON.parse(fs.readFileSync(checkttDataPath + checkttFile));
                        let storage = [], count = 1;
                        ...
                        let checkttBody = '==𝙏𝙊𝙋 𝙄𝙉𝙏𝙀𝙍𝘼𝘾𝙏 𝙄𝙈𝙈𝙄𝘿𝙄𝘼𝙏𝙀𝙇𝙔==\n\n';
                        checkttBody += storage.slice(0, 10).map(item => {
                            return `${count++}. ${item.name} with ${item.count} message`;
                        }).join('\n');
                        api.sendMessage(checkttBody, checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
    ...
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝙍𝘼𝙉 𝘽𝙊𝙏" : global.config.BOTNAME}`, "[ 𝗕𝗼𝘁 𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝘁𝗶𝗼𝗻 ]");
  ...
  const handleRefresh = require("./handle/handleRefresh")({  api, Threads, Users, Currencies, models });
    ...
    const checkTime = (time) => new Promise((resolve) => {
        ...
        if (time[1] > 12 || time[1] < 1) resolve("Your month appears to be invalid");
        if (time[0] > getDayFromMonth(time[1]) || time[0] < 1) resolve("Your date appears to be invalid");
        if (time[2] < 2022) resolve("What era do you live in?");
        if (time[3] > 23 || time[3] < 0) resolve("Your hours appear to be invalid");
        if (time[4] > 59 || time[3] < 0) resolve("Your minutes appear to be invalid");
    ...
    return async (event) => {
        if (event.type == "change_thread_image") api.sendMessage(`» [ 𝘽𝙀𝙎𝙏'  𝙂𝙍𝙊𝙐𝙋]\n»  ${event.snippet}`, event.threadID);
        switch (event.type) {
            case "message":
            case "message_reply":
            case "message_unsend":
